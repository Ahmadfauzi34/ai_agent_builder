...
node interactive_multi_input_ingress.mjs . /trusted/ingress-policy.json /private/ingress-ledger.json run-42 --allow-state-checkpoint-export
```

The final startup option explicitly allows the JSON Lines client to retrieve mutable layer state through `op=checkpoint`. Without it, checkpoint export is disabled; durable run receipts still bind the checkpoint byte digest internally.
...
With `--allow-state-checkpoint-export` enabled by the trusted host, `{"op":"restore","receipt_id":"sha256:..."}` loads only a checkpoint associated with a committed receipt for this subject. The host checks the private file, receipt ID, exact byte digest, and manifest digest; WASM imports into a new registry and verifies structural identity before the runner replaces the active session. Restore returns the manifest and program identity for inspection. It never restores external input tensors, signed proofs, or action authority. Bind all runtime ports again using new signed nonces and increasing revisions before `run`. Restore of a missing, tampered, or historical receipt without retained bytes fails, leaving the current session intact. A privileged host file writer remains inside the trust boundary.
