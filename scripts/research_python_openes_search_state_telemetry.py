...
FFI_DIR = REPO / "ffi"
REPORT = REPO / "python-openes-search-state-telemetry-report.json"
...
EXPECTED_MILESTONE_REWARDS = {
    48: -0.645007,
    96: -0.3909893173088647,
    144: -0.324997,
    192: -0.3092712352204114,
}
...
def numeric_summary(values: list[float]) -> dict[str, float]:
    if not values or not all(math.isfinite(value) for value in values):
        raise AssertionError("numeric telemetry summary requires finite non-empty values")
...
def train_case(
    ...
    for generation in range(generations):
        ...
        if str(report["strategy"]) != "openes_antithetic":
            raise AssertionError(f"case {label}: telemetry strategy mismatch")
    ...
    for interval_start, interval_end in INTERVALS:
        ...
        diversity_values = [float(item["diversity"]) for item in telemetry_slice]
        fitness_std_values = [float(item["fitness_std"]) for item in telemetry_slice]
        population_mean_values = [
            float(item["population_mean_fitness"]) for item in telemetry_slice
        ]
        generation_best_values = [
            float(item["generation_best_fitness"]) for item in telemetry_slice
        ]
        ...
        interval_reports.append(
            {
                ...
                "telemetry": {
                    "diversity": numeric_summary(diversity_values),
                    "fitness_std": numeric_summary(fitness_std_values),
                    "median_population_mean_fitness": statistics.median(
                        population_mean_values
                    ),
                    "median_generation_best_fitness": statistics.median(
                        generation_best_values
                    ),
                    "max_stagnation": max(
                        int(item["stagnation"]) for item in telemetry_slice
                    ),
                    "mean_norm_start": mean_norm_values[0],
                    "mean_norm_end": mean_norm_values[-1],
                    "mean_norm_delta": mean_norm_values[-1] - mean_norm_values[0],
    ...
    for generation, expected in EXPECTED_MILESTONE_REWARDS.items():
    ...
    final_improvement_generation = improvement_generations[-1]
    final_plateau_start = final_improvement_generation + 1
    final_plateau_telemetry = telemetry_history[final_improvement_generation:]
    if final_plateau_start <= generations and not final_plateau_telemetry:
        raise AssertionError("final champion plateau telemetry is empty")

    final_plateau_summary = None
    if final_plateau_telemetry:
        plateau_diversity = [
            float(item["diversity"]) for item in final_plateau_telemetry
        ]
        plateau_fitness_std = [
            float(item["fitness_std"]) for item in final_plateau_telemetry
        ]
        plateau_mean_norm = [
        ...
        final_plateau_summary = {
            "start_generation": final_plateau_start,
            "end_generation": generations,
            "length_generations": generations - final_improvement_generation,
            "diversity": numeric_summary(plateau_diversity),
            "fitness_std": numeric_summary(plateau_fitness_std),
            "mean_norm_start": plateau_mean_norm[0],
            "mean_norm_end": plateau_mean_norm[-1],
            "mean_norm_delta": plateau_mean_norm[-1] - plateau_mean_norm[0],
            "mean_norm_path_length": sum(
                abs(current - previous)
                for previous, current in zip(
                    plateau_mean_norm, plateau_mean_norm[1:]
                )
            ),
            "max_stagnation": max(
                int(item["stagnation"]) for item in final_plateau_telemetry
            ),
            "diversity_collapse_flags": plateau_flags.count(
                "DIVERSITY_COLLAPSE"
            ),
            "all_fitness_equal_flags": plateau_flags.count(
                "ALL_FITNESS_EQUAL"
            ),
    ...
    return (
        {
            ...
            "search_state_telemetry": telemetry_history,
            ...
            "final_champion_plateau": final_plateau_summary,
            "timing": {
...
final_plateau = trajectory_report["final_champion_plateau"]
if final_plateau is None:
    plateau_search_state = "NO_FINAL_CHAMPION_PLATEAU"
elif (
    int(final_plateau["diversity_collapse_flags"]) > 0
    or int(final_plateau["all_fitness_equal_flags"]) > 0
):
    plateau_search_state = "SEARCH_COLLAPSE_SIGNAL_ON_FINAL_CHAMPION_PLATEAU"
elif float(final_plateau["mean_norm_path_length"]) > 1e-6:
    plateau_search_state = "SEARCH_NORM_STILL_MOVING_ON_FINAL_CHAMPION_PLATEAU"
else:
    plateau_search_state = "SEARCH_NORM_STABLE_ON_FINAL_CHAMPION_PLATEAU"
...
report = {
    "verdict": "PASS",
    "schema": "burn-research.python-openes-search-state-telemetry.v1",
    ...
    "comparison": {
        ...
        "plateau_search_state": plateau_search_state,
    ...
    "proofs": {
        ...
        "tell_report_telemetry_finite": True,
        "milestone_continuity_with_depth_saturation": True,
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="burn-research-openes-search-state-telemetry-") as tmp:
        root = Path(tmp)
        wheels = root / "wheels"
        wheels.mkdir()
        run(
        ...
        if not lines:
            raise SystemExit("OpenES search-state telemetry consumer produced no output")
        payload = json.loads(lines[-1])
        if payload.get("verdict") != "PASS":
            raise SystemExit(f"unexpected OpenES search-state telemetry verdict: {payload}")
