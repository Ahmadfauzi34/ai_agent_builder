...
FFI_DIR = REPO / "ffi"
REPORT = REPO / "python-openes-learning-rate-schedule-report.json"
...
SCHEDULE_TARGET_LR = 0.08
CASES = [
    ("FIXED_010", 0.10, None),
    ("FIXED_008", 0.08, None),
    ("SCHEDULE_G96", 0.10, 96),
    ("SCHEDULE_G144", 0.10, 144),
]
MILESTONES = (48, 96, 144, 192)
CANDIDATE_SNAPSHOT_GENERATIONS = (97, 145)
...
def train_case(
...
) -> tuple[dict[str, object], list[float], dict[int, bytes]]:
    ...
    candidate_snapshots: dict[int, bytes] = {}
    transition_proof: dict[str, object] | None = None
    ...
    for generation in range(GENERATIONS):
        ...
        if generation_number in CANDIDATE_SNAPSHOT_GENERATIONS:
            candidate_snapshots[generation_number] = candidates.tobytes()
        ...
        if abs(float(report["lr"]) - active_learning_rate) > 1e-6:
            raise AssertionError(
                f"case {label}: telemetry learning-rate drift "
                f"{report['lr']} vs expected {active_learning_rate}"
            )
        ...
        if (
            ...
            and generation_number == transition_generation
        ):
            before_best = array("f", optimizer.best()).tobytes()
            before_program_identity = graph.program_identity()
            before_binding_identity = binding.identity()
            if abs(float(report["lr"]) - learning_rate) > 1e-6:
                raise AssertionError(
                    f"case {label}: transition report LR mismatch"
                )

            optimizer.set_learning_rate(SCHEDULE_TARGET_LR)

            after_best = array("f", optimizer.best()).tobytes()
            if after_best != before_best:
                raise AssertionError(
                    f"case {label}: LR transition changed champion state"
                )
            if graph.program_identity() != before_program_identity:
                raise AssertionError(
                    f"case {label}: LR transition changed program identity"
                )
            if binding.identity() != before_binding_identity:
                raise AssertionError(
                    f"case {label}: LR transition changed binding identity"
                )

            active_learning_rate = SCHEDULE_TARGET_LR
            transition_proof = {
                "transition_generation": transition_generation,
                "report_generation_before_setter": int(report["gen"]),
                "learning_rate_before": float(report["lr"]),
                "learning_rate_after_requested": SCHEDULE_TARGET_LR,
                "champion_bytes_unchanged": True,
                "program_identity_unchanged": True,
                "binding_identity_unchanged": True,
            }

        if (
            transition_proof is not None
            and generation_number == int(transition_proof["transition_generation"]) + 1
        ):
            transition_proof["first_post_transition_report_generation"] = int(
                report["gen"]
            )
            transition_proof["first_post_transition_report_lr"] = float(
                report["lr"]
            )
            if abs(float(report["lr"]) - SCHEDULE_TARGET_LR) > 1e-6:
                raise AssertionError(
                    f"case {label}: first post-transition report LR mismatch"
                )
    ...
    return (
        {
            ...
            "scheduled_learning_rate": (
                SCHEDULE_TARGET_LR if transition_generation is not None else None
            ),
...
with ExitStack() as stack:
    ...
    case_snapshots: dict[str, dict[int, bytes]] = {}

    for label, learning_rate, transition_generation in CASES:
        case_report, best_params, snapshots = train_case(
            stack,
            label,
            0.08,
            learning_rate,
        ...
        case_snapshots[label] = snapshots

    case_by_label = {str(item["label"]): item for item in case_reports}
    fixed010 = case_by_label["FIXED_010"]

    transition_cases = {
        "SCHEDULE_G96": 96,
        "SCHEDULE_G144": 144,
    }
    for label, transition_generation in transition_cases.items():
        report = case_by_label[label]
        proof = report.get("transition_proof")
        if not isinstance(proof, dict):
            raise AssertionError(f"case {label}: missing transition proof")

        first_post_generation = transition_generation + 1
        scheduled_bytes = case_snapshots[label].get(first_post_generation)
        fixed_bytes = case_snapshots["FIXED_010"].get(first_post_generation)
        if scheduled_bytes is None or fixed_bytes is None:
            raise AssertionError(
                f"case {label}: missing candidate snapshot at generation "
                f"{first_post_generation}"
            )
        if scheduled_bytes != fixed_bytes:
            raise AssertionError(
                f"case {label}: first post-transition candidates differ from FIXED_010"
            )
        proof["first_post_transition_candidate_bytes_match_fixed_010"] = True
        proof["first_post_transition_candidate_generation"] = first_post_generation

        for milestone in MILESTONES:
            if milestone <= transition_generation:
                scheduled_reward = float(
                    report["milestone_champion_rewards"][str(milestone)]
                )
                fixed_reward = float(
                    fixed010["milestone_champion_rewards"][str(milestone)]
                )
                if abs(scheduled_reward - fixed_reward) > 1e-9:
                    raise AssertionError(
                        f"case {label}: pre-transition milestone {milestone} "
                        f"diverged from FIXED_010"
                    )
...
fixed010_reward = float(case_by_label["FIXED_010"]["final_champion_reward"])
fixed008_reward = float(case_by_label["FIXED_008"]["final_champion_reward"])
g96_reward = float(case_by_label["SCHEDULE_G96"]["final_champion_reward"])
g144_reward = float(case_by_label["SCHEDULE_G144"]["final_champion_reward"])

fixed_best_label = max(
    ("FIXED_010", "FIXED_008"),
    key=lambda label: float(case_by_label[label]["final_champion_reward"]),
)
schedule_best_label = max(
    ("SCHEDULE_G96", "SCHEDULE_G144"),
    key=lambda label: float(case_by_label[label]["final_champion_reward"]),
)
final_leader = max(
    ("FIXED_010", "FIXED_008", "SCHEDULE_G96", "SCHEDULE_G144"),
    key=lambda label: float(case_by_label[label]["final_champion_reward"]),
)

schedule_best_reward = float(
    case_by_label[schedule_best_label]["final_champion_reward"]
)
fixed_best_reward = float(
    case_by_label[fixed_best_label]["final_champion_reward"]
)
schedule_beats_both_fixed = schedule_best_reward > fixed_best_reward

if schedule_beats_both_fixed:
    schedule_outcome = "SCHEDULE_BEATS_BOTH_FIXED_CONTROLS"
elif schedule_best_reward >= fixed010_reward and schedule_best_reward >= fixed008_reward:
    schedule_outcome = "SCHEDULE_MATCHES_BEST_FIXED_CONTROL"
else:
    schedule_outcome = "FIXED_CONTROL_REMAINS_BEST"

transition_preference = (
    "EARLIER_G96_SWITCH_LEADS"
    if g96_reward > g144_reward
    else "LATER_G144_SWITCH_LEADS"
    if g144_reward > g96_reward
    else "SCHEDULE_SWITCHES_TIE"
)

report = {
    "verdict": "PASS",
    "schema": "burn-research.python-openes-learning-rate-schedule.v1",
    ...
    "cases": case_reports,
    "comparison": {
        "fixed_010_final_reward": fixed010_reward,
        "fixed_008_final_reward": fixed008_reward,
        "schedule_g96_final_reward": g96_reward,
        "schedule_g144_final_reward": g144_reward,
        "fixed_best_case": fixed_best_label,
        "schedule_best_case": schedule_best_label,
        "final_leader": final_leader,
        "schedule_outcome": schedule_outcome,
        "transition_preference": transition_preference,
        "schedule_beats_both_fixed_controls": schedule_beats_both_fixed,
    ...
    "proofs": {
        ...
        "fixed_010_milestone_continuity": True,
        "state_preserving_setter_used": True,
        "pre_transition_schedule_matches_fixed_010": True,
        "first_post_transition_candidate_bytes_match_fixed_010": True,
        "transition_champion_state_unchanged": True,
        "transition_program_identity_unchanged": True,
        "transition_binding_identity_unchanged": True,
    ...
    "notes": [
        "All four cases use population 8, sigma 0.08, 192 generations, and exactly 1536 candidate evaluations.",
        "Scheduled cases start at LR 0.10 and call set_learning_rate(0.08) only after tell() completes at generation 96 or 144.",
        "The first post-transition candidate batch is byte-identical to FIXED_010 at the same generation, proving no optimizer restart at the schedule boundary.",
        "Only the host decides when to call the setter; no automatic schedule policy exists in the core.",
        "Case selection is evidence plumbing for exact ProgramBundle replay and does not promote a production optimizer setting.",
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="burn-research-openes-lr-schedule-") as tmp:
        root = Path(tmp)
        wheels = root / "wheels"
        wheels.mkdir()
        run(
        ...
        if not lines:
            raise SystemExit("OpenES learning-rate schedule consumer produced no output")
        payload = json.loads(lines[-1])
        if payload.get("verdict") != "PASS":
            raise SystemExit(
                f"unexpected OpenES learning-rate schedule verdict: {payload}"
            )
