...
FFI_DIR = REPO / "ffi"
REPORT = REPO / "python-nonlinear-es-depth-sigma-report.json"
...
with ExitStack() as stack:
    ...
    for label, population, generations in CASES:
...
d_reward = float(case_by_label["D_LOW_SIGMA"]["final_champion_reward"])
e_reward = float(case_by_label["E_LOW_SIGMA"]["final_champion_reward"])
if e_reward > d_reward:
    equal_768_pattern = "DEPTH_BETTER_AT_768_EVALS_AFTER_SIGMA_CORRECTION"
elif d_reward > e_reward:
    equal_768_pattern = "BREADTH_BETTER_AT_768_EVALS_AFTER_SIGMA_CORRECTION"
else:
    equal_768_pattern = "EQUAL_AT_768_EVALS_AFTER_SIGMA_CORRECTION"

c_reward = float(case_by_label["C_LOW_SIGMA"]["final_champion_reward"])
if e_reward > c_reward:
    depth_extension_pattern = "MORE_DEPTH_IMPROVES_LOW_SIGMA"
elif e_reward < c_reward:
    depth_extension_pattern = "MORE_DEPTH_DOES_NOT_IMPROVE_LOW_SIGMA"
else:
    depth_extension_pattern = "MORE_DEPTH_TIES_LOW_SIGMA"

report = {
    "verdict": "PASS",
    "schema": "burn-research.python-nonlinear-es-depth-sigma.v1",
    ...
    "cases": case_reports,
    "comparison": {
        "equal_768_pair": ["D_LOW_SIGMA", "E_LOW_SIGMA"],
        "equal_768_evaluation_count": 768,
        "equal_768_pattern": equal_768_pattern,
        "depth_extension_pair": ["C_LOW_SIGMA", "E_LOW_SIGMA"],
        "depth_extension_pattern": depth_extension_pattern,
    ...
    "proofs": {
        ...
        "candidate_evaluation_counts_exact": True,
    ...
    "notes": [
        "Each case uses an independent OpenES instance with the same seed, sigma=0.08, and learning_rate=0.06; only population/generation budget changes.",
        "The graph is reset to the same zero trainable state before every optimizer run.",
        "D_LOW_SIGMA and E_LOW_SIGMA both use exactly 768 candidate evaluations, isolating breadth versus depth after sigma correction.",
        "C_LOW_SIGMA exactly anchors the validated 8x48 low-sigma configuration from the preceding hyperparameter study.",
        "Case selection is only an evidence convenience for replay; it does not promote an optimizer policy or production champion.",
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="burn-research-nonlinear-es-depth-sigma-") as tmp:
        root = Path(tmp)
        wheels = root / "wheels"
        wheels.mkdir()
        run(
        ...
        if not lines:
            raise SystemExit("nonlinear ES depth/sigma consumer produced no output")
        payload = json.loads(lines[-1])
        if payload.get("verdict") != "PASS":
            raise SystemExit(f"unexpected nonlinear ES depth/sigma verdict: {payload}")
