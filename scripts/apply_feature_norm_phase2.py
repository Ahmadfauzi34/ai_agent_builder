...
# Protocol: allocate the next custom layer code.
replace(
    "src/protocol.rs",
...
# One canonical default shared by the typed facade and primitive.
replace(
...
# Registry: FeatureNorm becomes a first-class stateless unary family.
replace(
    "src/registry.rs",
...
init_ghost = "    fn init_ghost(&mut self, _header: &PacketHeader, payload: &[u8]) -> Result<(), String> {"
replace(
    "src/registry.rs",
    init_ghost,
...
# Typed agent facade. Omitted epsilon is canonicalized to the resolved default,
# so featureNorm(id) and featureNorm(id, 1e-12) have the same structural identity.
replace(
    "src/agent.rs",
...
feature_ctor = """    #[wasm_bindgen(js_name = featureNorm)]
    pub fn feature_norm(
        layer_id: u32,
        epsilon: Option<f64>,
...
replace(
    ...
    feature_ctor + "    #[wasm_bindgen(js_name = shiftUp)]\n",
)
replace(
    "src/agent.rs",
    r'\"adaptiveAvgPool2d\",\"shiftUp\"',
    r'\"adaptiveAvgPool2d\",\"featureNorm\",\"shiftUp\"',
)
replace(
    "src/agent.rs",
...
# Machine-readable layout and WASM surface inventories.
p = Path("docs/agent-layout-contracts.v1.json")
layout = json.loads(p.read_text())
layout["constructors"]["featureNorm"] = {
    "input": "feature_axis1_singleton",
    "output": "feature_axis1_singleton",
    "semantic": "L2 normalize feature axis 1; requires [B,F,1,1]",
}
p.write_text(json.dumps(layout, separators=(",", ":")) + "\n")

p = Path("docs/wasm-surface.v1.json")
surface = json.loads(p.read_text())
methods = surface["classes"]["AgentLayerSpec"]
if "featureNorm" not in methods:
    methods.append("featureNorm")
    methods.sort()
p.write_text(json.dumps(surface, separators=(",", ":")) + "\n")

# Cross-boundary Rust proof.
Path("src/feature_norm_integration_tests.rs").write_text(
    r'''#[cfg(test)]
mod tests {
    ...
    fn compiled_feature_norm(
        ...
        layer_id: u32,
        epsilon: Option<f64>,
    ) -> (AgentLayerSpec, AgentGraphBuilder, crate::graph::CompiledGraph) {
...
# Packaged-WASM proof through typed facade, CompiledGraph identity, and generic Program Bundle.
Path("scripts/research_feature_norm_integration.mjs").write_text(
