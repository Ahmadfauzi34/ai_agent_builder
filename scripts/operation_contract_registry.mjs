...
function layoutCompatibility(actualLayout, expectedLayout, compatibilityTable) {
  ...
  const compatibility = compatibilityTable?.[actualLayout];
...
function validateParameterType(value, type) {
  const optionalMatch = /^Option<(.+)>$/.exec(type);
  if (optionalMatch && value === null) return true;
  const normalized = optionalMatch?.[1] ?? type;
...
export function loadOperationContractRegistry(baseDir = MODULE_DIR) {
  ...
  const registry = {
    ...
    layout_definitions: layoutContract.layouts,
    layout_compatibility: layoutContract.compatibility,
    capacity: {
      graph_slots_max: graphPlan.invariants.graph_slots_max,
...
export function resolveDeclarativeGraphPlan(registry, plan) {
  ...
  for (const input of Array.isArray(plan.inputs) ? plan.inputs : []) {
    ...
    if (input.layout !== 'unknown' && !Object.prototype.hasOwnProperty.call(registry.layout_definitions, input.layout)) {
      diagnostics.push(diagnostic('LAYOUT_INCOMPATIBLE', 'layout', `input ${input.id} declares unknown layout ${input.layout}`));
      statuses.push('REJECTED');
    }
    if (['preserve_input', 'dynamic'].includes(input.layout)) {
      diagnostics.push(diagnostic('LAYOUT_INCOMPATIBLE', 'layout', `input ${input.id} cannot use internal relational layout ${input.layout}`));
      statuses.push('REJECTED');
    }
  ...
  for (const step of Array.isArray(plan.steps) ? plan.steps : []) {
    ...
    const stepId = typeof step?.id === 'string' && step.id.length > 0 ? step.id : null;
    ...
    if (typeof step?.operation === 'string') {
      ...
      const selectorQuery = {
        ...step.selector,
        arity: step.selector.arity ?? inputIds.length,
        input_layout: step.selector.input_layout ?? inputValues[0]?.layout,
      };
      const pool = queryOperationPool(registry, selectorQuery);
      candidates = pool.candidates
        .map(candidate => registry.operations.find(operation => operation.operation_id === candidate.operation_id))
        .filter(Boolean);
      if (pool.status === 'REJECTED') {
        ...
        stepStatuses.push('REJECTED');
      } else if (pool.status === 'AMBIGUOUS') {
        stepDiagnostics.push(diagnostic('OPERATION_SELECTION_AMBIGUOUS', 'selection', `step selector matches ${pool.candidate_count} operations`, stepId));
        stepStatuses.push('AMBIGUOUS');
      } else if (pool.status === 'DEFERRED') {
    ...
    if (selected) {
      ...
      for (let index = 0; index < inputValues.length; index++) {
        ...
        const relation = layoutCompatibility(value.layout, expectedLayout, registry.layout_compatibility);
    ...
    if (!outputId) {
      stepDiagnostics.push(diagnostic('INVALID_PARAMETER', 'graph', 'step output must be a non-empty string', stepId));
      stepStatuses.push('REJECTED');
    } else if (values.has(outputId)) {
      stepDiagnostics.push(diagnostic('INVALID_PARAMETER', 'graph', `step output ${outputId} collides with an existing value`, stepId));
      stepStatuses.push('REJECTED');
    } else {
      values.set(outputId, {
