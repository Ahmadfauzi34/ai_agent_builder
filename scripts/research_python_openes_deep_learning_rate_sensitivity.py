...
FFI_DIR = REPO / "ffi"
REPORT = REPO / "python-openes-deep-learning-rate-sensitivity-report.json"
...
low_reward = float(case_by_label["LR_LOW"]["final_champion_reward"])
base_reward = float(case_by_label["LR_BASE"]["final_champion_reward"])
high_reward = float(case_by_label["LR_HIGH"]["final_champion_reward"])

final_leader = max(
    ("LR_LOW", "LR_BASE", "LR_HIGH"),
...
if final_leader == "LR_LOW":
    sensitivity_pattern = "LOW_LEARNING_RATE_LEADS_AT_192"
elif final_leader == "LR_HIGH":
    sensitivity_pattern = "HIGH_LEARNING_RATE_LEADS_AT_192"
else:
    sensitivity_pattern = "BASE_LEARNING_RATE_LEADS_AT_192"
...
schedule_hypothesis = early_leader != "LR_LOW" and final_leader == "LR_LOW"

report = {
    "verdict": "PASS",
    "schema": "burn-research.python-openes-deep-learning-rate-sensitivity.v1",
    ...
    "comparison": {
        "base_case": "LR_BASE",
        "learning_rate_cases": ["LR_LOW", "LR_BASE", "LR_HIGH"],
        "low_final_reward": low_reward,
        "base_final_reward": base_reward,
        "high_final_reward": high_reward,
        ...
        "schedule_hypothesis_supported": schedule_hypothesis,
    ...
    "proofs": {
        ...
        "base_milestone_continuity": True,
    ...
    "notes": [
        ...
        "Only learning rate changes across LR_LOW, LR_BASE, and LR_HIGH.",
        "LR_BASE milestones reproduce the validated corrected-sigma lineage.",
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="burn-research-openes-deep-lr-") as tmp:
        root = Path(tmp)
        wheels = root / "wheels"
        wheels.mkdir()
        run(
        ...
        if not lines:
            raise SystemExit("deep OpenES learning-rate sensitivity consumer produced no output")
        payload = json.loads(lines[-1])
        if payload.get("verdict") != "PASS":
            raise SystemExit(
                f"unexpected deep OpenES learning-rate sensitivity verdict: {payload}"
            )
