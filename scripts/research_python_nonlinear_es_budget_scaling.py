...
FFI_DIR = REPO / "ffi"
REPORT = REPO / "python-nonlinear-es-budget-scaling-report.json"
...
b_reward = float(case_by_label["B"]["final_champion_reward"])
c_reward = float(case_by_label["C"]["final_champion_reward"])
if b_reward > c_reward:
    equal_eval_pattern = "BREADTH_BETTER_AT_384_EVALS"
elif c_reward > b_reward:
    equal_eval_pattern = "DEPTH_BETTER_AT_384_EVALS"
else:
    equal_eval_pattern = "EQUAL_AT_384_EVALS"

a_reward = float(case_by_label["A"]["final_champion_reward"])
d_reward = float(case_by_label["D"]["final_champion_reward"])
if d_reward > a_reward:
    budget_pattern = "LARGER_BUDGET_IMPROVES_FINAL_REWARD"
elif d_reward < a_reward:
    budget_pattern = "LARGER_BUDGET_DOES_NOT_IMPROVE_FINAL_REWARD"
else:
    budget_pattern = "LARGER_BUDGET_TIES_BASE_BUDGET"

report = {
    "verdict": "PASS",
    "schema": "burn-research.python-nonlinear-es-budget-scaling.v1",
    ...
    "cases": case_reports,
    "comparison": {
        "equal_evaluation_pair": ["B", "C"],
        "equal_evaluation_count": 384,
        "equal_eval_pattern": equal_eval_pattern,
        "base_vs_largest_budget": ["A", "D"],
    ...
    "notes": [
        "Each case uses an independent OpenES instance with the same seed and hyperparameters except population/generation budget.",
        "The graph is reset to the same zero trainable state before every optimizer run.",
        "B and C both use exactly 384 candidate evaluations, isolating breadth versus depth.",
        ...
        "No optimizer algorithm, ABI, graph/controller, activation surface, or support claim changes in this research.",
    ],
}
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="burn-research-nonlinear-es-budget-") as tmp:
        root = Path(tmp)
        wheels = root / "wheels"
        wheels.mkdir()
        run(
        ...
        if not lines:
            raise SystemExit("nonlinear ES budget consumer produced no output")
        payload = json.loads(lines[-1])
        if payload.get("verdict") != "PASS":
            raise SystemExit(f"unexpected nonlinear ES budget verdict: {payload}")
