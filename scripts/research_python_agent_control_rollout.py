...
FFI_DIR = REPO / "ffi"
REPORT = REPO / "python-agent-control-rollout-report.json"
...
def build_policy(stack: ExitStack):
    ...
    layer = stack.enter_context(
        host.LinearLayerSpec(210_000, POLICY_IN, POLICY_OUT, bias=True)
    )
...
decision = (
    ...
    else "HOST_OR_POLICY_FRICTION_FOUND"
)
...
report = {
    ...
    "notes": [
        ...
        "No new primitive, ABI symbol, graph controller, or support claim is introduced.",
        "Decision thresholds classify the research result only; they are not performance SLAs.",
    ],
}
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="burn-research-agent-rollout-") as tmp:
        root = Path(tmp)
        wheels = root / "wheels"
        wheels.mkdir()
        run(
        ...
        if not lines:
            raise SystemExit("agent rollout research produced no output")
