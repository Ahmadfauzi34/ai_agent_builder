...
FFI_DIR = REPO / "ffi"
REPORT = REPO / "python-nonlinear-es-search-budget-report.json"
...
breadth_reward = case_by_label["B"]["final_champion_reward"]
depth_reward = case_by_label["C"]["final_champion_reward"]
equal_eval_preference = (
    "BREADTH"
    if breadth_reward > depth_reward
    else "DEPTH"
    if depth_reward > breadth_reward
    else "TIE"
)

base_reward = case_by_label["A"]["final_champion_reward"]
best_reward = selected["final_champion_reward"]
budget_gain = best_reward - base_reward
if selected["label"] == "A":
    decision = "MORE_SEARCH_BUDGET_DID_NOT_IMPROVE_FINAL_REWARD"
elif budget_gain > 0.10:
    decision = "NONLINEAR_POLICY_SEARCH_BUDGET_SCALING_VISIBLE"
else:
    decision = "NONLINEAR_POLICY_SEARCH_BUDGET_GAIN_SMALL"
...
report = {
    ...
    "comparison": {
        ...
        "selected_final_reward": best_reward,
        "selected_minus_base_reward": budget_gain,
        "equal_evaluation_cases": ["B", "C"],
        "equal_evaluation_preference": equal_eval_preference,
        "breadth_B_final_reward": breadth_reward,
        "depth_C_final_reward": depth_reward,
        "B_minus_C_reward": breadth_reward - depth_reward,
    },
    "replay": {
    ...
    "notes": [
        "Only population and generation count vary across cases A-D.",
        "B and C both use 384 candidate evaluations, isolating breadth versus depth at equal evaluation count.",
        "Training-quality decisions are research interpretation only and are not CI pass/fail thresholds.",
        "No optimizer algorithm, ABI symbol, activation surface, graph semantic, or support claim changes in this slice.",
    ],
}
